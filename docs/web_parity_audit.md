...
- Web Audio output discovery through the browser device APIs when sink
  ...
  request/error surfacing, unavailable-Web-Audio refresh/connect errors,
  resume request, Orbifold audio connected state,
...
- A final gate report exists, without a passing
  `./scripts/check-web-parity-complete.mjs reports/` run over the saved
  evidence.
